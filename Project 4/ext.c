...
// remake if the minPopVoteToWinFast function for an electoral tie
MinInfo minPopVoteToWinFast_exact(State* states, int szStates) {
    // initialize variables
    int totEVs = totalEVs(states,szStates);
    int reqEVs = totEVs/2; // required EVs to win election
...
int main(int argc, char* argv[]) {
    ...
    // Extension:
    // --------------------------------------------------------------------------------------------------------------
    printf("Q1: Are you interested in finding out what the winning stats are with fewer states? (yes/no)\n");
    char choice1[5];
    scanf("%s", choice1);
    printf("\n");
    if(strcmp(choice1, "yes") == 0){
        printf("How many states would you like to include in the new calculation?\n");
        int numStates = 0;
        scanf("%d", &numStates);

        toWin = minPopVoteToWinFast(allStates, numStates);
        totEVs = totalEVs(allStates, numStates);
        wonEVs = totalEVs(toWin.someStates,toWin.szSomeStates);
        reqEVs = totEVs / 2 + 1;
        totPVs = totalPVs(allStates, numStates);
    ...
    printf("Q2: Are you interested in looking at an example of an exact Electoral College tie? (yes/no)\n");
    char choice2[5];
    scanf("%s", choice2);
    printf("\n");
    if(strcmp(choice2, "yes") == 0){
        printf("Here is some data relating to an exact tie for this year:\n");

        toWin = minPopVoteToWinFast_exact(allStates, nStates);
        totEVs = totalEVs(allStates, nStates);
        wonEVs = totalEVs(toWin.someStates,toWin.szSomeStates);
        reqEVs = totEVs / 2;
        totPVs = totalPVs(allStates, nStates);
