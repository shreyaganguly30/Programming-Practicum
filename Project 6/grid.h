...
Description: This program creates a labyrinth that is either a square or is twisty.
In EscapeTheLabyrinth.h, there is a function that takes in a string of moves. It
then checks if the moves allow the player to collect all 3 artifacts. If the player
gets all 3, they escape. If they don't, they don't escape. In grid.h, we have the
configuration for the maze grid. One of the 2 labyrinths is a square, so here are the
constructor, setter, and getter methods for the grid. For instance, one of the getter
methods checks to see what is at a position. This is helpful in knowing if the
artifact was found or not.
